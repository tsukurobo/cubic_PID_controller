...
//#define SLAVE_ADDRESS_ENCODER 0x08 
//#define ENABLE_PIN 2
//SPIピンの番号
//MISO 50
//MOSI 51
...
uint8_t g_buf[10];//RP2040への送信データを格納する配列。クラス内変数として定義したかったが上手くいかなかった。

SPISettings mySPISettings = SPISettings(4000000, MSBFIRST, SPI_MODE0);


//初期セットアップを行う関数
void Cubic_motor::begin(int8_t motornumber){ //制御したいモータを0~7の整数で指定してもらう

    //Wire.begin(); //I2C通信セットアップ
    ...
    if(0 <= motornumber && motornumber <8){ //正常な引数値が与えられた時
...
bool Cubic_encoder::operator >> (int &enc){
    ...
    if (Wire.available() >= 4){ //回転数を受信できた時
        for(int i = 0; i < 4; i++) rp += Wire.read() << i*8; //受信したデータをrpに格納
        enc = rp; //encにrpの値を代入
